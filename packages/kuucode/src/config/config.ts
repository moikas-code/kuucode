...
export namespace Config {
  ...
  export const Keybinds = z
    .object({
      ...
      project_init: z.string().optional().default("<leader>i").describe("Create/update .agentrc"),
  ...
  export const global = lazy(async () => {
    ...
    await import(path.join(Global.Path.config, "config"), {
      ...
      .then(async (mod) => {
        ...
        result["$schema"] = "https://kuucode.ai/config.json"
  ...
  async function load(configPath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://kuucode.ai/config.json"
