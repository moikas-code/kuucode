...
if (!snapshot) {
  ...
  const pkgbuild = [
    ...
    "url='https://github.com/sst/kuucode'",
    "arch=('aarch64' 'x86_64')",
    "license=('MIT')",
    "provides=('opencode')",
    "conflicts=('opencode')",
    "depends=('fzf' 'ripgrep')",
    "",
    `source_aarch64=("\${pkgname}_\${pkgver}_aarch64.zip::https://github.com/sst/kuucode/releases/download/v${version}/opencode-linux-arm64.zip")`,
    `sha256sums_aarch64=('${arm64Sha}')`,
    "",
    `source_x86_64=("\${pkgname}_\${pkgver}_x86_64.zip::https://github.com/sst/kuucode/releases/download/v${version}/opencode-linux-x64.zip")`,
  ...
  for (const pkg of ["kuucode", "opencode-bin"]) {
  ...
  const homebrewFormula = [
    ...
    `  homepage "https://github.com/sst/kuucode"`,
    `  version "${version.split("-")[0]}"`,
    "",
    "  on_macos do",
    "    if Hardware::CPU.intel?",
    `      url "https://github.com/sst/kuucode/releases/download/v${version}/opencode-darwin-x64.zip"`,
    `      sha256 "${macX64Sha}"`,
    "",
    "      def install",
    '        bin.install "kuucode"',
    ...
    "    end",
    "    if Hardware::CPU.arm?",
    `      url "https://github.com/sst/kuucode/releases/download/v${version}/opencode-darwin-arm64.zip"`,
    `      sha256 "${macArm64Sha}"`,
    "",
    "      def install",
    '        bin.install "kuucode"',
    ...
    `      url "https://github.com/sst/kuucode/releases/download/v${version}/opencode-linux-x64.zip"`,
    `      sha256 "${x64Sha}"`,
    "      def install",
    '        bin.install "kuucode"',
    "      end",
    ...
    `      url "https://github.com/sst/kuucode/releases/download/v${version}/opencode-linux-arm64.zip"`,
    `      sha256 "${arm64Sha}"`,
    "      def install",
    '        bin.install "kuucode"',
    "      end",
