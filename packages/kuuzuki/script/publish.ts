...
if (!snapshot) {
  ...
  const previous = await fetch("https://api.github.com/repos/moikas-code/kuuzuki/releases/latest")
    .then((res) => {
  ...
  const commits = await fetch(`https://api.github.com/repos/moikas-code/kuuzuki/compare/${previous}...HEAD`)
    .then((res) => {
      if (!res.ok) throw new Error(res.statusText)
      return res.json()
    })
  ...
  const pkgbuild = [
    ...
    "depends=('fzf' 'ripgrep')",
    "",
    `source_aarch64=("\${pkgname}_\${pkgver}_aarch64.zip::https://github.com/moikas-code/kuuzuki/releases/download/v${version}/kuuzuki-linux-arm64.zip")`,
    `sha256sums_aarch64=('${arm64Sha}')`,
    "",
    `source_x86_64=("\${pkgname}_\${pkgver}_x86_64.zip::https://github.com/moikas-code/kuuzuki/releases/download/v${version}/kuuzuki-linux-x64.zip")`,
  ...
  const homebrewFormula = [
    ...
    `      url "https://github.com/moikas-code/kuuzuki/releases/download/v${version}/kuuzuki-darwin-x64.zip"`,
    `      sha256 "${macX64Sha}"`,
    "",
    "      def install",
    '        bin.install "kuuzuki"',
    ...
    "    end",
    "    if Hardware::CPU.arm?",
    `      url "https://github.com/moikas-code/kuuzuki/releases/download/v${version}/kuuzuki-darwin-arm64.zip"`,
    `      sha256 "${macArm64Sha}"`,
    "",
    "      def install",
    '        bin.install "kuuzuki"',
    ...
    `      url "https://github.com/moikas-code/kuuzuki/releases/download/v${version}/kuuzuki-linux-x64.zip"`,
    `      sha256 "${x64Sha}"`,
    "      def install",
    '        bin.install "kuuzuki"',
    "      end",
    ...
    `      url "https://github.com/moikas-code/kuuzuki/releases/download/v${version}/kuuzuki-linux-arm64.zip"`,
    `      sha256 "${arm64Sha}"`,
    "      def install",
    '        bin.install "kuuzuki"',
    "      end",
