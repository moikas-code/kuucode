...
export const TuiCommand = cmd({
  command: "$0 [project]",
  describe: "start kuucode tui",
  builder: (yargs) =>
    yargs
      .positional("project", {
        type: "string",
        describe: "path to start kuucode in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    while (true) {
      ...
      const result = await bootstrap({ cwd }, async (app) => {
        ...
        let cmd = ["./cmd/kuucode/kuucode"]
        let cwd = Bun.fileURLToPath(new URL("../../../../tui/", import.meta.url))
      ...
      if (result === "needs_provider") {
        ...
        const result = await Bun.spawn({
          cmd: [...getKuucodeCommand(), "auth", "login"],
          cwd: process.cwd(),
          stdout: "inherit",
          stderr: "inherit",
          stdin: "inherit",
...
/**
 * Get the correct command to run kuucode CLI
 * In development: ["bun", "run", "packages/kuucode/src/index.ts"]
 * In production: ["/path/to/kuucode"]
 */
