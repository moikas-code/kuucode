---
title: Intro
description: Get started with kuuzuki.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

[**kuuzuki**](/) is an AI coding agent built for the terminal.

![kuuzuki TUI with the kuuzuki theme](../../../assets/lander/screenshot.png)

Let's start by installing kuuzuki.

---

## Install

The easiest way to install kuuzuki is through the install script.
...
- **Using Node.js**
  ...
  <Tabs>
    <TabItem label="npm">```bash npm install -g kuuzuki-ai ```</TabItem>
    <TabItem label="Bun">```bash bun install -g kuuzuki-ai ```</TabItem>
    <TabItem label="pnpm">```bash pnpm install -g kuuzuki-ai ```</TabItem>
    <TabItem label="Yarn">```bash yarn global add kuuzuki-ai ```</TabItem>
  </Tabs>

- **Using Homebrew on macOS**

  ```bash
  brew install moikas-code/tap/kuuzuki
  ```
...
Right now the automatic installation methods do not work properly on Windows. However you can grab the binary from the [Releases](https://github.com/moikas-code/kuuzuki/releases).
...
Once you've signed up, run `kuuzuki auth login` and select Anthropic.
...
This will get kuuzuki to analyze your project and create an `AGENTS.md` file in
the project root.
...
For more straightforward changes, you can ask kuuzuki to directly build it
...
You want to make sure you provide a good amount of detail so kuuzuki makes the right
changes.
...
Here's an [example conversation](https://kuuzuki.ai/s/4XP1fce5) with kuuzuki.
...
To make it your own, we recommend [picking a theme](/docs/themes), [customizing the keybinds](/docs/keybinds), or playing around with the [kuuzuki config](/docs/config).
