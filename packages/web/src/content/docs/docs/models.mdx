...
kuucode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

---

## Providers

You can configure providers in your kuucode config under the `provider` section.

---

### Defaults

Most popular providers are preloaded by default. If you've added the credentials for a provider through `kuucode auth login`, they'll be available when you start kuucode.
...
```json title="kuucode.json" {5,9-11}
{
...
When kuucode starts up, it checks for the following:

1. The model list in the kuucode config.
