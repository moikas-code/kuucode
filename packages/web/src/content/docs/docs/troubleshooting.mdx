...
To debug any issues with kuucode, you can check the logs or the session data
...
kuucode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/kuucode/`
- **Windows**: `%USERPROFILE%\.local\share\kuucode`
...
If you're experiencing issues with kuucode:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/moikas-code/kuucode/issues**](https://github.com/moikas-code/kuucode/issues)
...
2. **Join our Discord**
   ...
   [**kuucode.ai/discord**](https://kuucode.ai/discord)
...
3. Ensure you have the latest version with `kuucode upgrade`

---

### Authentication issues

1. Try re-authenticating with `kuucode auth login <provider>`
...
kuucode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
