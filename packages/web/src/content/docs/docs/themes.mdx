...
With kuucode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, kuucode uses our own `kuucode` theme.
...
kuucode comes with several built-in themes.
...
- Want kuucode to match their terminal's appearance
...
kuucode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/kuucode/themes/*.json` or `$XDG_CONFIG_HOME/kuucode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.kuucode/themes/*.json`
4. **Current working directory** - Defined in `./.kuucode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/kuucode/themes
vim ~/.config/kuucode/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .kuucode/themes
vim .kuucode/themes/my-theme.json
```
...
{
  "$schema": "https://kuucode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
