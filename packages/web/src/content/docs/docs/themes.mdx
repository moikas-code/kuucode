...
With kuuzuki you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, kuuzuki uses our own `kuuzuki` theme.
...
kuuzuki comes with several built-in themes.
...
- Want kuuzuki to match their terminal's appearance
...
kuuzuki supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/kuuzuki/themes/*.json` or `$XDG_CONFIG_HOME/kuuzuki/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.kuuzuki/themes/*.json`
4. **Current working directory** - Defined in `./.kuuzuki/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/kuuzuki/themes
vim ~/.config/kuuzuki/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .kuuzuki/themes
vim .kuuzuki/themes/my-theme.json
```
...
{
  "$schema": "https://kuuzuki.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
